...
Alternatively, which is what we adopt in this paper, for simplicity,
we can simply give a name to an instance, in an instance declaration,
and use that name in import and export clauses (see
\cite{Controlling-scope-instances}). Inthis paper we don't even need
to give a name to an instance, since we only need to consider
instances (of undeclared classes) with a single member. We can thus
use the member name as the instance name.
